...
body {
    margin: 0;
    padding: 24px;
    background: radial-gradient(circle at top, #10151f, var(--bg));
    color: var(--text);
...
/* Panels */
.panel {
    background: linear-gradient(180deg, #121a27, var(--panel));
...
<div class="grid grid-3 fade-in">
    <div class="panel">
        ...
        <div class="meter"><div class="high" style="width: $([math]::Min(100,$highRisk*10))%"></div></div>
    </div>
    <div class="panel">
        <strong>MEDIUM RISK</strong> ($medRisk)
        <div class="meter"><div class="med" style="width: $([math]::Min(100,$medRisk*10))%"></div></div>
    </div>
    <div class="panel">
        <strong>LOW RISK</strong> ($lowRisk)
        <div class="meter"><div class="low" style="width: $([math]::Min(100,$lowRisk*10))%"></div></div>
    </div>
</div>
        ...
        if ($items.Count -gt 0) {
            ...
            foreach ($item in $items) {
                ...
                if ($item.PSObject.Properties.Name -contains "RiskScore") {
                    ...
                    if ($r -ge 80) { $cls = "risk-high" }
                    elseif ($r -ge 50) { $cls = "risk-med" }
                    elseif ($r -gt 0) { $cls = "risk-low" }
                }
